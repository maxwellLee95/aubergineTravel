...
<head>
    <meta charset="utf-8">
    <title> {if $ordertype=='all'}全部团购订单{elseif $ordertype=='unpay'}未付款团购订单{else}未点评团购订单{/if}-{$webname}</title>
...
<script>
    $(function(){
        ...
        //导航选中
        $('#nav_visaorder').addClass('on');

        //订单类型切换
        $(".tabnav span").click(function(){
            var orderType = $(this).attr('data-type');
            var url = SITEURL+'member/order/visa-'+orderType;
