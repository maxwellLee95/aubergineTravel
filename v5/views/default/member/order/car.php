...
<head>
    <meta charset="utf-8">
    <title> {if $ordertype=='all'}全部车辆订单{elseif $ordertype=='unpay'}未付款车辆订单{else}未点评车辆订单{/if}-{$webname}</title>
...
<script>
    $(function(){
        ...
        //导航选中
        $('#nav_carorder').addClass('on');

        //订单类型切换
        $(".tabnav span").click(function(){
            var orderType = $(this).attr('data-type');
            var url = SITEURL+'member/order/car-'+orderType;
