...
class Model_Line extends ORM {
    ...
    public function cloneLine($id, $num)
    {
        ...
        for($i = 1; $i <= $num; $i++)
        {
         ...
         //   $arr['linepic']=$this->clonePicture($arr['linepic'],$arr['webid']);
            ...
            $line = ORM::factory('line',$new_line_id);
            $line->aid=$new_line_id;
            $line->save();
            //如果是新增，则自动创建套餐
            $data_arr = array();
            $data_arr['suitname'] = '默认';
            $data_arr['lineid'] =$new_line_id;
    ...
    * */
    ...
    {
	    $newtime = time();
        $sql = "SELECT MIN(adultprice) as price FROM sline_line_suit_price WHERE lineid='$lineid' and day>=$newtime";
    ...
    {
        $sql = "SELECT * FROM sline_line WHERE aid={$id}";
