...
class Controller_Order extends Stourweb_Controller{
    ...
    public function action_index()
    {
        ...
        if(empty($action))  //显示列表
        {
            ...
            if ($typeid==Model_Leader::TYPE_ID){
                $this->display('stourtravel/order/bountyu_list');
            }else{
        ...
        else if($action=='update')//更新某个字段
        {
            ...
            if($model->id)
            {
                ...
                if($model->saved())
                {
                    ...
                    }else if ($field=='status' && $val==Model_Member_Order::STATUS_FINISH ){
    ...
    public function action_view()
    {
        ...
        if($typeid==1)
        {
            ...
            if ($info['marshal_point_id']){
                $info['marshal_point']=ORM::factory('marshalpoint',$info['marshal_point_id'])->as_array();
            }
        }
    ...
    public function action_ajax_save()
    {
        ...
        if($model->saved())
        {
            ...
            {
                if($oldstatus!=Model_Member_Order::STATUS_CANCEL && $current_status==Model_Member_Order::STATUS_CANCEL)
                {
                    Model_Member_Order::refundStorage($id,'plus');//订单取消,增加库存
                    Model_Distributionrecord::cancelDistributionByOrder($id);
                }
                else if($oldstatus==Model_Member_Order::STATUS_CANCEL && $current_status==Model_Member_Order::STATUS_ACCEPTANCE) //由取消变为在处理中
                {
                    Model_Member_Order::refundStorage($id,'minus');//订单增加,库存减少
                }

                if($current_status==Model_Member_Order::STATUS_FINISH)
                {
                    Model_Member_Order::refundJifen($id);
                    if ($model->typeid==Model_Line::TYPE_ID){
                        Model_Distributionrecord::useDistributionByOrder($id);
                    }
                }

                if ($current_status==Model_Member_Order::STATUS_PAY){
                    Common::paySuccess($model->ordersn);
                }

                //通知
                Model_Msg::msgOrderFinish($id);
                Model_Msg::msgOrderPay($id);
                Model_Msg::msgOrderCancel($model->id,'用户申请取消，后台审核通过');
                Model_Msg::msgDistributionSettlement($id);
    ...
    public function action_genexcel()
    {
        ...
        foreach($arr as $row)
        {
            ...
            if($order['typeid']!=2)
            {
                ...
                {
                    $price = $price-intval($order['jifentprice']);//减去学分抵现的价格.
                }
            ...
            if($typeid==1)
            {
                ...
                $tourers = ORM::factory('member_order_tourer')->where("orderid","=",$row['id'])->get_all();

                $marshalPoint = ORM::factory('marshalpoint')->where("id","=",$row['marshal_point_id'])->find()->as_array();
                $t=[];
                if($tourers){
                    foreach($tourers as $tourer){
                        $t[]=$tourer['tourername'].','.$tourer['cardnumber'].','.$tourer['mobile'];
                    }
                }
                $s=implode("||",$t);
                $table.="<td>{$s}</td>";
                $table.="<td>{$marshalPoint['name']}</td>";
            }
            $table.="<td>{$price}</td>";
