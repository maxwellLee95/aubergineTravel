...
class Controller_Mall extends Stourweb_Controller
{
    ...
    public function before()
    {
        ...
        if ($action == 'mall')
        {
            ...
            if (!empty($user_action))
                Common::getUserRight('mall', $user_action);
        }
        else if ($action == 'add')
        {
            Common::getUserRight('mall', 'sadd');
        }
        else if ($action == 'edit')
        {
            Common::getUserRight('mall', 'smodify');
        }
        else if ($action == 'price')
        {
            Common::getUserRight('mallprice', 'slook');
        }
        else if ($action == 'ajax_price')
        {
            ...
            if ($op == 'save')
            {
                Common::getUserRight('mallprice', 'smodify');
            }
            else if ($op == 'del')
            {
                Common::getUserRight('mallprice', 'sdel');
            }
        }
        else if ($action == 'rank' || $action == 'ajax_ranklist')
        {
            Common::getUserRight('mallrank', 'slook');
        }
        else if ($action == 'ajax_rank_add')
        {
            Common::getUserRight('mallrank', 'sadd');
        }
        else if ($action == 'ajax_rank_save')
        {
            Common::getUserRight('mallrank', 'smodify');
        }
        else if ($action == 'ajax_rank_del')
        {
            Common::getUserRight('mallrank', 'sdel');
        }
        $this->assign('parentkey', $this->params['parentkey']);
        $this->assign('itemid', $this->params['itemid']);
        $this->assign('weblist', Common::getWebList());
        $this->assign('templetlist', Common::getUserTemplteList('mall_show'));//获取上传的用户模板
    }
    ...
    */
    public function action_mall()
    {
        $action = $this->params['action'];
        if (empty($action))  //显示商品列表页
        {
            $this->assign('kindmenu', Common::getConfig('menu_sub.mallkind'));//分类设置项
            $this->display('stourtravel/mall/list');
        }
        else if ($action == 'read')    //读取列表
        {
            ...
            $w .= empty($rankid) ? '' : " and a.mallrankid='$rankid'";
            $w .= $webid == '-1' ? '' : " and a.webid=$webid";
            if (empty($kindid))
            {
                $sql = "select a.id,a.aid,a.title,a.kindlist,a.attrid,a.mallrankid,a.mallrankid ,a.finaldestid,a.ishidden,a.webid,a.iconlist,a.supplierlist,a.themelist,b.isding,b.isjian,b.istejia,if(length(ifnull(a.attrid,''))=0,0,1) as order_attrid,if(length(ifnull(a.kindlist,''))=0,0,1) as order_kindlist,if(length(ifnull(a.iconlist,''))=0,0,1) as order_iconlist,if(length(ifnull(a.themelist,''))=0,0,1) as order_themelist
,ifnull(b.displayorder,9999) as displayorder,ifnull(d.suitday,0) as suitday from sline_mall as a left join sline_allorderlist b on (a.id=b.aid and b.typeid=".Model_Mall::TYPE_ID.") left join (select c.mallid,c.id,min(c.suitday) as suitday from(select a.mallid,a.id,max(b.day) as suitday
 from sline_mall_sku a left join sline_mall_sku_price b on a.id=b.suitid  group by a.id) c group by c.mallid) d on a.id=d.mallid  where $w $order limit $start,$limit";
            }
            else
            {
                $sql = "select a.id,a.aid,a.title,a.kindlist,a.attrid,a.mallrankid,a.mallrankid ,a.finaldestid,a.ishidden,a.webid,a.iconlist,if(length(ifnull(a.attrid,''))=0,0,1) as order_attrid,if(length(ifnull(a.kindlist,''))=0,0,1) as order_kindlist,if(length(ifnull(a.iconlist,''))=0,0,1) as order_iconlist,if(length(ifnull(a.themelist,''))=0,0,1) as order_themelist
,a.themelist,b.isding,b.isjian,b.istejia,ifnull(b.displayorder,9999) as displayorder,ifnull(d.suitday,0) as suitday from sline_mall as a left join sline_kindorderlist as b on (b.classid=$kindid and a.id=b.aid and b.typeid=".Model_Mall::TYPE_ID.") left join (select c.mallid,c.id,min(c.suitday) as suitday from(select a.mallid,a.id,max(b.day) as suitday
 from sline_mall_sku a left join sline_mall_sku_price b on a.id=b.suitid  group by a.id) c group by c.mallid) d on a.id=d.mallid where $w $order limit $start,$limit";
            }
            $totalcount_arr = DB::query(Database::SELECT, "select count(*) as num from sline_mall a where $w")->execute()->as_array();
            ...
            foreach ($list as $k => $v)
            {
                ...
                $v['attrname'] = Model_Mall_Attr::getAttrnameList($v['attrid']);

                $v['url'] = Common::getBaseUrl($v['webid']) . '/malls/show_' . $v['aid'] . '.html';
                ...
                $v['iconname'] = $name;
                $v['series'] = Common::getSeries($v['id'], '27');//编号
                //供应商信息
                ...
                $v['qq'] = $supplier['qq'];
                //$homes=ORM::factory('mall_sku')->where("mallid",'=',$v['id'])->get_all(); //读取套餐
                $suitOrder = $sort[0]['property'] == 'suitday' ? 'order by suitday ' . $sort[0]['direction'] : '';
                $homeSql = "select a.*,ifnull(max(b.day),0) as suitday from sline_mall_sku a left join sline_mall_sku_price b on a.id=b.suitid where a.mallid={$v['id']}  group by a.id $suitOrder";
        ...
        else if ($action == 'delete') //删除某个记录
        {
            ...
            if (is_numeric($id)) //商品
            {
                $model = ORM::factory('mall', $id);
                $model->deleteClear();
            }
            else if (strpos($id, 'suit') !== FALSE)
            {
                $suitid = substr($id, strpos($id, '_') + 1);
                $suit = ORM::factory('mall_sku', $suitid);
                $mallid = $suit->mallid;
                $suit->deleteClear();
                Model_Mall::updateMinPrice($mallid);
            }
        }
        else if ($action == 'update')//更新某个字段
        {
            ...
            if ($field == 'displayorder')  //如果是排序
            {
                ...
                if (is_numeric($id))//如果是商品
                {
                    if (empty($kindid))  //全局排序
                    {
                        $order = ORM::factory('allorderlist');
                        $order_mod = $order->where("aid='$id' and typeid=".Model_Mall::TYPE_ID." and webid=0")->find();
                        ...
                        else   //如果这个排序不存在
                        {
                            ...
                            $order_mod->aid = $id;
                            $order_mod->webid = 0;
                            $order_mod->typeid = Model_Mall::TYPE_ID;
                        }
                        $order_mod->save();
                        if ($order_mod->saved())
                        {
                    ...
                    else  //按目的地排序
                    {
                        ...
                        $kindorder_mod = $kindorder->where("aid='$id' and typeid=".Model_Mall::TYPE_ID." and classid=$kindid")->find();
                        if ($kindorder_mod->id)
                        {
                        ...
                        else
                        {
                            ...
                            $kindorder_mod->typeid = Model_Mall::TYPE_ID;
                        }
                        $kindorder_mod->save();
                        if ($kindorder->saved())
                            echo 'ok';
            ...
            else  //如果不是排序字段
            {
                ...
                {
                    $suitid = substr($id, strpos($id, '_') + 1);
                    $model = ORM::factory('mall_sku', $suitid);
                }
                if ($model->id)
                {
                    $model->$field = $val;
                    ...
                    {
                        $model->$field = implode(',', Model_Attrlist::getParentsStr($val, Model_Mall::TYPE_ID));
                    }
                    $model->save();
                    if ($model->saved())
                        echo 'ok';
    ...
    {
        $columns = ORM::factory('mall_content')->where("(webid=0 and isopen=1) or columnname='tupian'")->order_by('displayorder', 'asc')->get_all();
        $this->assign('columns', $columns);
        $this->assign('webid', 0);
        $this->assign('position', '添加商品');
        $this->assign('action', 'add');
        $this->assign('ranklist', ORM::factory('mall_rank')->getList());
        $this->display('stourtravel/mall/edit');
    }
    ...
    public function action_edit()
    {
        ...
        $extendinfo = Common::getExtendInfo(Model_Mall::TYPE_ID, $info['id']);
        $this->assign('columns', $columns);
        $this->assign('extendinfo', $extendinfo);//扩展信息
        $this->assign('info', $info);
        $this->assign('position', '修改商品' . $info['title']);
        $this->display('stourtravel/mall/edit');
    }
    ...
    public function action_ajax_save()
    {
        ...
        {
            $model = ORM::factory('mall');
            $model->aid = Common::getLastAid('sline_mall', $webid);
            $model->addtime = time();
        }
        else
        {
            ...
            {
                $aid = Common::getLastAid('sline_mall', $webid);
                $model->aid = $aid;
            }
            $productid = $id;
        }
        ...
        if (!empty($attrids))
        {
            $attrmode = ORM::factory("mall_attr")->where("id in ($attrids)")->group_by('pid')->get_all();
        ...
        $model->modtime = time();
        $model->mallrankid = Arr::get($_POST, 'mallrankid');
        $model->lng = $_POST['lng'];
        $model->lat = $_POST['lat'];
        ...
        if ($model->saved())
        {
            ...
            Common::saveExtendData(Model_Mall::TYPE_ID, $productid, $_POST);//扩展信息保存
            $status = true;
        }
    ...
    {
        $this->display('stourtravel/mall/rank');
    }
    ...
    {
        $model = new Model_Mall_Rank();
        $arr = $model->order_by('orderid', 'asc')->get_all();
        $out = array();
        foreach ($arr as $row)
        {
            $out[] = array('displayorder' => $row['orderid'], 'rankname' => $row['mallrank'], 'id' => $row['id']);
        }
    ...
    public function action_ajax_rank_add()
    {
        ...
        $model = ORM::factory('mall_rank');
        $model->mallrank = $rankname;
        $model->orderid = '9999';
        $model->webid = 0;
        $model->create();
        $flag = 0;
    ...
    public function action_ajax_rank_save()
    {
        ...
        {
            $obj = ORM::factory('mall_rank')->where('id', '=', $id[$i])->find();
            $obj->mallrank = $rankname[$i];
    ...
    public function action_ajax_rank_del()
    {
        ...
        $model = ORM::factory('mall_rank', $navid);
        $model->delete();
        $out = array();
        if (!$model->loaded())
        {
    ...
    public function action_price()
    {
        ...
        if (empty($action))
        {
            $list = ORM::factory('mall_pricelist')->where('webid', '=', '0')->order_by('min', 'asc')->get_all();
            $this->assign('list', $list);
            $this->display('stourtravel/mall/price');
        }
        if ($action == 'save')
        {
    ...
    * */
    public function action_ajax_price()
    {
        ...
        if ($action == 'add')
        {
            /*$row = ORM::factory('mall_pricelist')
                ->where('webid','=','0')
                ->order_by('id','desc')
                ->limit(1)
                ->find()->as_array();
            ...
            $model = new Model_Mall_Pricelist();
            $model->min = $min;
            $model->webid = 0;
            $flag = 0 ;
            $priceid = 0;
        ...
        if ($action == 'save')
        {
            ...
            {
                $obj = ORM::factory('mall_pricelist')->where('id', '=', $id[$i])->find();
                $obj->min = $min[$i];
                $obj->max = $max[$i];
                $obj->update();
                $obj->clear();
            ...
            {
                $obj = ORM::factory('mall_pricelist');
                $obj->min = $newmin[$i];
                $obj->max = $newmax[$i];
                $obj->webid = 0;
                $obj->create();
        ...
        {
            $id = Arr::get($_POST, 'id');
            $model = ORM::factory('mall_pricelist', $id);
            $model->delete();
            $out = array();
            if (!$model->loaded())
            {
    ...
    public function action_room()
    {
        ...
        $this->assign('mallid', $mallid);
        if (empty($action))  //显示线路列表页
        {
            $info = ORM::factory('mall', $mallid)->as_array();
            $position = $info['title'];
            $this->assign('position', $position);
            $this->display('stourtravel/mall/room');
        }
        else if ($action == 'read')    //读取列表
        {
            $start = Arr::get($_GET, 'start');
            $limit = Arr::get($_GET, 'limit');
            /*$list = ORM::factory('mall_sku')
                       ->where('mallid','=',$mallid)
                       ->order_by('displayorder','asc')
                       ->offset($start)
                       ->limit($limit)
                       ->as_array();*/
            $sql = "select a.* from sline_mall_sku a where a.mallid='$mallid' order by a.id desc limit $start,$limit";
            $totalcount_arr = DB::query(Database::SELECT, "select count(*) as num from sline_mall_sku a where mallid='$mallid'")->execute()->as_array();
            $list = DB::query(Database::SELECT, $sql)->execute()->as_array();
            $new_list = array();
            foreach ($list as $k => $v)
            {
    ...
    public function action_room_op()
    {
        ...
        $mallid = $this->params['mallid'];
        $this->assign('mallid', $mallid);
        $mallinfo = ORM::factory('mall', $mallid)->as_array();
        $this->assign('mallname', $mallinfo['title']);
        if ($action == 'add')
        {
            $info = array('lastoffer' => array('pricerule' => 'all'));
            $this->assign('position', '添加规格');
            $this->assign('action', 'add');
        ...
        else if ($action == 'edit')
        {
            ...
            $info = ORM::factory('mall_sku', $roomid)->as_array();
        ...
        $this->display('stourtravel/mall/room_edit');
    }
    ...
    public function action_ajax_room_save()
    {
        ...
        $mallid = Arr::get($_POST, 'mallid');
        $images = Arr::get($_POST, 'images');
        $imagestitle = Arr::get($_POST, 'imagestitle');
        $roomid = Arr::get($_POST, 'roomid');//房间id
        $description = Arr::get($_POST, 'description');//规格说明
        $suitid = $roomid;
        //图片处理
        $piclist = '';
        for ($i = 1; isset($images[$i]); $i++)
        ...
        {
            $model = ORM::factory('mall_sku');
        }
        else //修改保存
        {
            $model = ORM::factory('mall_sku', $roomid);
        }
        $model->name = Arr::get($_POST, 'name');
        ...
        $model->number = Arr::get($_POST, 'number') ? Arr::get($_POST, 'number') : 0;
        ...
        $model->lastoffer = Common::last_offer(Model_Mall::TYPE_ID, $_POST);
        if ($action == 'add' && empty($roomid))
        {
            $model->create();
        }
        ...
        self::saveBaoJia($mallid, $suitid, $_POST);
     ...
     * 商品规格报价
     * */
    public function saveBaoJia($mallid, $roomid, $arr)
    {
        //$pricerule,$starttime,$endtime,$mallid,$roomid,$basicprice,$profit,$description
        ...
        if ($pricerule == 'all')
        {
            ...
            while (true)
            {
                $model = ORM::factory('mall_sku_price')->where("suitid=$roomid and day='$begintime'")->find();
                $data_arr = array();
                $data_arr['mallid'] = $mallid;
                ...
                if ($model->suitid)
                {
                    $query = DB::update('mall_sku_price')->set($data_arr)->where("suitid=$roomid and day='$begintime'");
                    $query->execute();
                }
                else
                {
            ...
            while (true)
            {
                ...
                foreach ($monthval as $v)
                {
                    ...
                    if ((int)$v > (int)$daynum || $newtime < $stime || $newtime > $etime)
                        continue;
                    $model = ORM::factory('mall_sku_price')->where("suitid=$roomid and day='$newtime'")->find();
                    $data_arr = array();
                    $data_arr['mallid'] = $mallid;
                    $data_arr['suitid'] = $roomid;
                    $data_arr['basicprice'] = $basicprice;
                    ...
                    if ($model->suitid)
                    {
                        $query = DB::update('mall_sku_price')->set($data_arr)->where("suitid=$roomid and day='$newtime'");
                        $query->execute();
                    }
                    else
                    {
    ...
    public function action_ajax_clone()
    {
        ...
        $id = Arr::get($_POST, 'id');
        $model = new Model_Mall();
        $flag = $model->cloneMall(intval($id), intval($num));
