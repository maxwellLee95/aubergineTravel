...
class Controller_Calendar extends Stourweb_Controller
{
    ...
    public function action_index()
    {
        ...
        $all_leaders=array();
        if ($typeid==1){
//            $suit_info=Model_Line_Suit::suit_info($suitid);
//            $line=Model_Line::detail($suit_info['lineid']);
            $all_leaders_data=Model_Leader::get_all_leaders();
            if ($all_leaders_data){
                foreach ($all_leaders_data as $v){
                    $all_leaders[$v['id']]=$v;
                }
            }
        }
        ...
        $this->assign('all_leaders', $all_leaders);
...
//        $number = (string)$number == '不限' ? -1 : $number;
        ...
        $leader_id2 = !empty($_POST['leader_id2']) ? $_POST['leader_id2'] : null;
        $leader_id3 = !empty($_POST['leader_id3']) ? $_POST['leader_id3'] : null;
        $leader_id4 = !empty($_POST['leader_id4']) ? $_POST['leader_id4'] : null;
        $leader_id5 = !empty($_POST['leader_id5']) ? $_POST['leader_id5'] : null;
        ...
        if ($typeid == 1)
        {
            ...
            $add_update .= ", leader_id='$leader_id'";
            $add_update .= ", leader_id2='$leader_id2'";
            $add_update .= ", leader_id3='$leader_id3'";
            $add_update .= ", leader_id4='$leader_id4'";
            $add_update .= ", leader_id5='$leader_id5'";
        ...
        if ($flag)
        {
            ...
            $out['leader_id2'] = $leader_id2;
            $out['leader_id3'] = $leader_id3;
            $out['leader_id4'] = $leader_id4;
            $out['leader_id5'] = $leader_id5;
        ...
        foreach ($arr as $row)
        {
            if ($row)
            {
                ...
                if ($typeid == 1)
                {
                    ...
                    //leader_id
                    $price[$day]['leader_id'] = $row['leader_id'];
                    $price[$day]['leader_id2'] = $row['leader_id2'];
                    $price[$day]['leader_id3'] = $row['leader_id3'];
                    $price[$day]['leader_id4'] = $row['leader_id4'];
                    $price[$day]['leader_id5'] = $row['leader_id5'];
                }
...
';
        ...
        while ($day <= $lastday)
        {
            ...
            //单价
            $dayPrice = isset($priceArr[$cdayme]['price'])?$priceArr[$cdayme]['price']:null;
            //成本
            ...
            //库存
            $number = $priceArr[$cdayme]['number'];


            $leader_id = $priceArr[$cdayme]['leader_id'];
            $leader_id2 = $priceArr[$cdayme]['leader_id2'];
            $leader_id3 = $priceArr[$cdayme]['leader_id3'];
            $leader_id4 = $priceArr[$cdayme]['leader_id4'];
            $leader_id5 = $priceArr[$cdayme]['leader_id5'];
            ...
            {
                if (($dayPrice || $day_child_price || $day_old_price) || ($typeid==1 && !is_null($dayPrice)))
                {
                    if ($dayPrice > 0 || ($typeid==1 && !is_null($dayPrice)))
                    {
