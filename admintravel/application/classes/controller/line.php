...
c.lineid,c.id,min(c.suitday) as suitday from(select a.lineid,a.id,max(ifnull(b.day,0)) as suitday from sline_line_suit a left join sline_line_suit_price b on a.id=b.suitid group by a.id) c group by c.lineid) d on a.id=d.lineid where $w $order  limit $start,$limit";
    ...
    public function action_add()
    {
        ...
        $this->assign('index_module_arr', Model_Indexmodule::get_index_module_arr());
        $this->assign('theme_arr', Model_Theme::get_arr());
        $this->assign('difficulty_list', Common::difficulty_list());
    ...
    public function action_edit()
    {
        ...
        if ($model->id)
        {
            ...
            $info['themelist_arr'] = $info['themelist']?explode(",",$info['themelist']):array();
            $info['iconlist_arr'] = Common::getSelectedIcon($info['iconlist']);
            $info['marshal_point_arr'] = Common::getSelectedMarshalPoint($info['marshal_point']);
            ...
            $info['index_module_ids'] = $info['index_module_ids']?explode(",",$info['index_module_ids']):array();
            $this->assign('index_module_arr', Model_Indexmodule::get_index_module_arr());
            $this->assign('theme_arr', Model_Theme::get_arr());
            $this->assign('difficulty_list', Common::difficulty_list());
            $this->assign('columns', $columns);
    ...
    */
    public function action_ajax_linesave()
    {
        ...
        $data_arr['line_type'] = Arr::get($_POST, 'line_type') ? Arr::get($_POST, 'line_type') : 1;
        $data_arr['difficultylevel'] = Arr::get($_POST, 'difficultylevel') ? Arr::get($_POST, 'difficultylevel') : 1;
        $data_arr['qrcode'] = Arr::get($_POST, 'qrcode');
        $data_arr['qrcode_text'] = Arr::get($_POST, 'qrcode_text');
        $data_arr['insurance_tips'] = Arr::get($_POST, 'insurance_tips');
        $data_arr['marshal_point'] = Arr::get($_POST, 'marshal_point') ? implode(',', Arr::get($_POST, 'marshal_point')) : '';
        $data_arr['person_count'] = Arr::get($_POST, 'person_count');
        $data_arr['min_person_count'] = Arr::get($_POST, 'min_person_count');
        $data_arr['custom_promotion_link'] = Arr::get($_POST, 'custom_promotion_link');
        $data_arr['share_desc'] = Arr::get($_POST, 'share_desc');

        $data_arr['index_module_ids'] = Arr::get($_POST, 'index_module_ids') ? implode(',', Arr::get($_POST, 'index_module_ids')) : 10;
        $data_arr['index_title'] = Arr::get($_POST, 'index_title');
        $data_arr['index_sub_title'] = Arr::get($_POST, 'index_sub_title');
        $data_arr['index_litpic'] = Arr::get($_POST, 'index_litpic');

        $data_arr['themelist'] = Arr::get($_POST, 'themelist') ? implode(',', Arr::get($_POST, 'themelist')) : '';
//        print_r($data_arr);
//        exit;
        //图片处理
...
//                $aid = Common::getLastAid('sline_line', $data_arr['webid']);
            ...
            if ($model->qrcode!==$data_arr['qrcode']){
                $model->qrcode_up_time = time();
            }
            $model->modtime = time();
        }
        ...
        if ($model->saved())
        {
            ...
            if ($lineid == 0){
                $data_arr = array();
                $data_arr['suitname'] = '默认';
                $data_arr['lineid'] = $model->id;
                $data_arr['propgroup'] = 2;
                $data_arr['jifentprice'] = 0;
                $data_arr['jifenbook'] = 0;
                $data_arr['jifencomment'] = 0;
                $data_arr['paytype'] = 1;
                $line_suit = ORM::factory('line_suit');
                foreach ($data_arr as $k => $v)
                {
                    $line_suit->$k = $v;
                }
...
//            $this->savelinemodule($lineid);
    ...
    public function savelinemodule($lineid){
        $line_module_id=Arr::get($_POST, 'line_module_id');
        $index_module_line_id=Arr::get($_POST, 'index_module_line_id');

        if ($lineid){
            if ($line_module_id){
//                if ($index_module_line_id){
//                    $model = ORM::factory('indexmoduleline',$index_module_line_id);
//                }else{
//                    $model = ORM::factory('indexmoduleline');
//                }
                $model = ORM::factory('indexmoduleline')->where("line_id={$lineid}")->find();
                if (empty($model->id)){
                    $model = ORM::factory('indexmoduleline');
                }
                $model->line_id=$lineid;
                $model->title=Arr::get($_POST, 'index_module_title');
                $model->sub_title=Arr::get($_POST, 'index_module_sub_title');
                $model->image=Arr::get($_POST, 'logo');
                $model->line_module_id=Arr::get($_POST, 'line_module_id');
...
//                    echo $model->id;
                ...
                Model_Indexmoduleline::deleteClearByline($lineid);
            }
   ...
   */
    ...
    public function saveBaoJia($lineid, $suitid, $arr)
    {
        ...
        $this->_deleteSuitPrice($lineid, $suitid);
        //按日期范围报价
        if ($pricerule == 'all')
        {
            $begintime = $stime;
    ...
    public function _deleteSuitPrice($lineid,$suitid){
        $date=date('Y-m-d');
        $time=strtotime($date);
        $model_list=ORM::factory('line_suit_price')->where("suitid={$suitid} and lineid={$lineid} And day>=$time")->find_all();
        $sql="select DISTINCT usedate from sline_member_order WHERE typeid=1 AND productaid={$lineid} AND suitid={$suitid} AND  usedate>={$date}";
        $member_order_date=DB::query(Database::SELECT,$sql)->execute()->as_array();
        $member_order_date_list=array();
        if ($member_order_date){
            foreach ($member_order_date as $item){
                $member_order_date_list[]=$item['usedate'];
            }
        }

        if ($model_list){
            foreach ($model_list as $model){
                if (!empty($model->suitid) && $model->day){
                    $cur_date=date('Y-m-d',$model->day);
                    if (!in_array($cur_date,$member_order_date_list)){
                        DB::delete('line_suit_price')->where("lineid={$lineid} and suitid={$suitid} and day=$model->day")->execute();
                    }
                }
            }
        }
