...
class Controller_Upgrade extends Stourweb_Controller
{
    ...
    private function backupOriginalFile($unzippath)
    {
        ...
        while (!feof($fp))
        {
            ...
            $filename = str_ireplace('admintravel', $GLOBALS['cfg_backdir'], $filename);
    ...
    private function replaceBackgroundDir($unzippath)
    {
        ...
        $backdir = $GLOBALS['cfg_backdir'] ? $GLOBALS['cfg_backdir'] : 'admintravel';

        if (strtoupper($backdir) != strtoupper('admintravel') && is_dir($unzippath))
        {
            ...
            while ($object = readdir($dh))
            {
                if ($object != "." && $object != "..")
                {
                    ...
                    if (is_dir($fullname))
                    {
                        if (strtoupper($object) == strtoupper('admintravel'))
                        {
