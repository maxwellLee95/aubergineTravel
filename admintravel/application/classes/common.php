...
//                break;
    ...
    */
    ...
    public static function getSelectedMarshalPoint($str)
    {
        $id_arr = explode(',', $str);
        $arr = array();
        foreach ($id_arr as $k => $v)
        {
            $data = ORM::factory('marshalpoint', $v);
            if ($data->id)
                $arr[] = $data->as_array();
        }
        return $arr;
    ...
    public static function getSelectedTheme($str)
    {
        $id_arr = explode(',', $str);
        $arr = array();
        foreach ($id_arr as $k => $v)
        {
            $data = ORM::factory('theme', $v);
            if ($data->id)
                $arr[] = $data->as_array();
        }
        return $arr;
...
//                if ($memberModel->save())
//                {
//                    self::addJifenLog($arr['memberid'], "预订{$arr['productname']}获得学分{$addjifen}", $addjifen, 2,$orderModel->id);
//                }
//            }
    ...
    //积分操作，TODO 事务
    public static function operateIntegral($memberid,$integral,$content,$type,$action=Model_Member_Jifen_Log::ACTION_OTHER,$param=array()){
        $flag=Model_Member::operate_jifen($memberid, $integral, $type);
        if ($flag){
            Model_Member_Jifen_Log::addLog($memberid, $content, $integral, $type,$action,$param);
            return true;
        }
        return false;
    }
    ...
    //leader积分操作，TODO 事务
    public static function leaderOperateIntegral($leader_id,$integral,$content,$type,$action=Model_Leader_Integral_Log::ACTION_OTHER,$param=array()){
        $flag=Model_Leader::operateIntegral($leader_id, $integral, $type);
        if ($flag){
            Model_Leader_Integral_Log::addLog($leader_id, $content, $integral, $type,$action,$param);
            return true;
        }
        return false;
    }
