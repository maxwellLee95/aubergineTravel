...
Product={
	...
	,getMarshalPoint:function(dom,selector)
    {
        //alert(5);
        var str='';
        $(selector+" input:hidden").each(function(index,ele){
        ...
        CHOOSE.setSome("设置集合点",{loadWindow:window,loadCallback:Product.listMarshalPoint},SITEURL+'marshalpoint/dialog_seticon?list='+str+'&selector='+encodeURI(selector),1);
    },
    listMarshalPoint:function(result,bool)
    {
        var html="";
        var arr=result.data;
        for(var i in arr)
        {
            html+="<span><s onclick=\"$(this).parent('span').remove()\"></s>"+arr[i].name+"<input type='hidden' name='marshal_point[]' value='"+arr[i].id+"'/></span>";
            $(result.selector).html(html);
        }
    }
    ,getTheme:function(id,type)
    {
        CHOOSE.setSome("设置专题",{loadWindow:window,loadCallback:Product.listTheme},SITEURL+'theme/dialog_settheme?typeid='+type+'&id='+id,1);
    },
    listTheme:function(result,bool)
    {
        var html="";
        var arr=result.data;
        for(var i in arr)
        {
            html+="<span><s onclick=\"$(this).parent('span').remove()\"></s>"+arr[i].ztname+"<input type='hidden' name='themelist[]' value='"+arr[i].id+"'/></span>";
            $(result.selector).html(html);
        }
    },
