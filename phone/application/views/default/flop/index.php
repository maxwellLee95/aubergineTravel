...
<div class="content">
    ...
    <div class="class-list" style="height: 478px;">
        ...
        {loop $line_list $k $item}
        <div class="class-content {if $k==1}active {/if}">
            <div class="class-content-main" style="height: 468px;">
                {loop $item $row}
                <div class="class-line" style="width:100%;height: 100%">
                    <a href="{$row['url']}">
                        <div style="position:relative;">
                            <img class="class-line-img" src="{Common::img($row['litpic'])}">
                            <div class="class-line-money">{Common::price($row['price'])}</div>
                        </div>
                        <div class="line-name">{$row['title']}</div>
                        <div class="line-info">{$row['lineday']}天&nbsp;&nbsp;难度:{Common::difficulty_desc($row['difficultylevel'])}</div>
                    </a>
                </div>
                {/loop}
