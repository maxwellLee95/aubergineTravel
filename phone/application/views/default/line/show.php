...
<head>
    <meta charset="utf-8">
    <title>活动详情-{$GLOBALS['cfg_webname']}</title>
    {if $seoinfo['keyword']}
...
<div class="content">
    ...
    <div class="activity-img">
        <a href="#">
            <img class="activity-banner" src="{$info['litpic']}" style="width:100%;">
            <div class="activity-name">{$info['title']}</div>
        </a>
    </div>
    <!-- 活动信息 -->
    <div class="activity-infos">
        <div class="activity-leader-info">
            <div class="pull-left" style="position:relative;">
                <a href="#">
                    <img class="img-circle" src="{Common::member_pic($info['leader']['litpic'])}" style="width:50px; height:50px;">
                    <img src="/phone/public/images/wx/v4/list/leader_star_{$info['leader']['star']}.png" class="leader-level">
                </a>
            </div>
            <div class="pull-left" style="padding:5px 10px;">
                <div>
                    <div class="activity-leader">{$info['leader']['nickname']}</div>
        ...
        <div class="activity-info">
            <div>难度:&nbsp;<span>{Common::difficulty_desc($info['difficultylevel'])}</span>&nbsp;&nbsp;<span>{if $info['jifenbook']}+{$info['jifenbook']}学分{/if}</span></div>
        </div>
        <div class="activity-info" style="padding-bottom:10px; height:34px;">
                ...
                <p>
                    ...
                    <select id="select-map" >
                        {loop $info['marshal_point_list'] $row}
                            <option value="{$row['id']}" data-longitude="{$row['longitude']}" data-latitude="{$row['latitude']}">{$row['name']}</option>
                        {/loop}
                    </select>
                </p>
            </div>
            <div class="pull-right">
                {if !empty($info['price'])}
                <div><div class="activity-money">{Currency_Tool::symbol()}{$info['price']}</div></div>
                {/if}
            </div>
            <div class="clear"></div>
        </div>
        {if $GLOBALS['cfg_open_baidumap'] && $info['marshal_point_list'][0]}
        <div class="map">
        <iframe id="baidumap" style="border: none" src="/phone/pub/baidumap?longitude={$info['marshal_point_list'][0]['longitude']}&latitude={$info['marshal_point_list'][0]['latitude']}" width="350px" height="225px" ></iframe>
        </div>
        {/if}
    </div>
    <!-- 出发时间 -->
                    ...
                    <div class="swiper-container time-activitys-swiper active" data-index="0" data-initial="1">
                        <div class="swiper-wrapper">
                            ...
                            <div class="swiper-slide" data-index="" data-month="">
                                <a href="{Common::_detail_url($info['aid'],$row['suitid'],$row['day'])}">
                                    <div class="other-activity">
                                        <div class="other-activity-main {if $row['day']==$info['day'] && $row['suitid']==$info['suitid']} checked {/if}">
                                            <div class="other-activity-content">
                                                <div class="other-activity-time">
                                                    <span>{date('m月d',$row['day'])}</span>
                                                    <span style="padding-left:5px;">{Common::date_text_week($row['day'])}</span>
                                                </div>
                                                <div class="other-activity-status">
                                                                <span style="    background-color: rgba(53,179,112,0.05);" class="{$row['status']['style']}">{$row['status']['name']} | 剩{$row['number']}名额</span>
                                                </div>
                                                {if $row['day']==$info['day'] && $row['suitid']==$info['suitid']}
                                        ...
                                        <div class="right-lian">
                                            <div class="left-circle {if $row['day']==$info['day'] && $row['suitid']==$info['suitid']} checked {/if}"></div>
                                            <div class="right-circle {if $row['day']==$info['day'] && $row['suitid']==$info['suitid']} checked {/if}"></div>
    ...
    <div class="activity-each">
        <a class="each-title" href="/phone/line/line_suit_tourer?aid={$info['id']}&suitid={$info['suitid']}&day={$info['day']}">
            <div class="pull-left">
                已参加的人
                ( <span style="color:#9B8CE5;">{$info['min_person_count']} </span>人成行 )
            </div>
            <div class="pull-right">
                总共<span style="color:#9B8CE5;"><?php echo $info['sell_numm']?></span>/{$info['person_count']}人&nbsp;<img src="/phone/public/images/wechat/v4/right_h.png" style="height:10px; margin-bottom:2px;">
            </div>
            <div class="clear"></div>
        </a>
                    ...
                    <div class="hiker-img">
                        <div style="position:relative; display:inline-block;">
                            <div style="position:relative; display:inline-block; overflow:hidden; border-radius:50%;">
                                <img class="img-circle" src="{Common::member_pic($row['litpic'])}" style="width:50px; height:50px;">
                            </div>
                            <img src="/phone/public/images/wx/v4/list/leader_star_{$row['star']}.png" class="leader-level">
                        </div>
                    </div>
                    <div class="hiker-name" style="color:red;font-size: 14px;" >{$row['nickname']}</div>
                </a>
            </div>
            {/loop}
            {/if}
                    ...
                    <div class="hiker-img">
                        <div style="position:relative; display:inline-block;">
                            <div style="position:relative; display:inline-block; overflow:hidden; border-radius:50%;">
                                ...
                                {if $row['dingnum']>1}
                                    <div class="join-num">
                                        <div>{$row['dingnum']}人</div>
                                    </div>
                                {/if}
                            </div>
                            <span style="position: absolute;right: -7px;top: 0;width: 16px;" class="icon_css {if $row['sex']=='男'}icon_man{/if}{if $row['sex']=='女'}icon_woman{/if}"></span>
                        </div>
                    </div>
    ...
    <div class="bottom-btns">
        ...
        <div id="buy" class="bottom-btn join" style="width:30%;{if !$info['signUpBtnStatus']}background-color: #ccc;{/if}">
            <!-- 判断是否可以显示倒计时 -->
            ...
            <input id="sign-up-tips" type="hidden" value="{$info['signUpTips']}">
            {if $info['signUpBtnStatus']}
                <a id="showSignupBtn" href="{$info['bookUrl']}" >{$info['signUpBtnText']}</a>
            {else}
                <a id="disableSignUpBtn" href="javascript::void(0)" >{$info['signUpBtnText']}</a>
            {/if}
        </div>
        <div class="clear"></div>
    </div>
    ...
    <div class="modal-body">
        ...
        <div class="mbox" id="wechatGroupQrcodeBox">
            ...
            <div style="margin-top: 5px;">
                <span class="grey">
                                                                        欢迎长按二维码<br>{$info['qrcode_text']}</span>
            </div>
            <div class="clr"></div>
...
<script type="text/javascript">
    ...
    var shareTitle = "<?php echo $info['title']?>",
        ...
        customize_qrcode = "{$customize_qrcode}";
