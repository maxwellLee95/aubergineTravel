...
<script>
    $(function(){
        ...
        function load(){
            ...
            $.getJSON(SITEURL + 'leader/integral/ajax_logs', page, function (data) {
                if (data.status) {
                    var html = '';
                    for (var i = 0; i < data['list'].length; i++) {
                        ...
                        if (item['type'] == 1) {
                            html += '<div class="num"><p class="green">-'+item['integral']+'</p></div>';
                        }
                        else {

                            html += '<div class="num"><p class="red">+'+item['integral']+'</p></div>';
                        }
                        html += '</li>';
                    }
                    if (page.current > 1) {
