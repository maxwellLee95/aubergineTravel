...
<div id="scoreDet" class="page out">
    <div class="header_top bar-nav">
        ...
        <h1 class="page-title-bar">赏金明细</h1>
    </div>
    <div style="padding: 10px 40px;    float: left;"><a href="/phone/leader/bounty/withdraw" >我要提现>></a> </div>
    <div style="padding: 10px 40px;    float: right;"><a href="/phone/leader/bounty/list" >提现记录>></a> </div>
    <div style="clear:both;"></div>
    <!-- 公用顶部 -->
    <div>
        <div style="display: none" class="score-nav">
            <ul class="clearfix" id="log_nav">
                <li data="0"><a href="#">全部</a></li>
                <li data="2"><a href="#">已完成</a></li>
                <li data="1"><a href="#">进行中</</a></li>
            </ul>
        </div>
...
<script>
    $(function(){
        ...
        function load(){
            ...
            $.getJSON('/leader/bounty/ajax_order_list', page, function (data) {
                if (data.status) {
                    var html = '';
                    for (var i = 0; i < data['list'].length; i++) {
                        var item = data['list'][i];
                        html += '<li><div class="txt">';
                        html += ' <strong class = "name" >'+item['member']['nickname']+'</strong > ';
                        html += '<span class="date">' + item['addtime'] + '</span></div>';
                        html += '<div class="num">'+item['price']+'</div>';
                        html += '</li>';
                    }
                    if (page.current > 1) {
