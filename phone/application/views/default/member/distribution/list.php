...
<div id="scoreDet" class="page out">
    <div class="header_top bar-nav">
        ...
        <h1 class="page-title-bar">分销记录</h1>
    </div>
    <div style="padding: 10px 40px;    float: right;"><a href="/phone/member/commission/list" >查看佣金明细>></a> </div>
    ...
    <div>
        ...
        <div class="score-nav">
            <ul class="clearfix" id="log_nav">
                <li data="2"><a href="#">已结算</a></li>
                <li data="0"><a href="#">未结算</a></li>
...
<script>
    $(function(){
        ...
        function load(){
            ...
            $.getJSON(SITEURL + 'member/distribution/ajax_list', page, function (data) {
                if (data.status) {
                    var html = '';
                    for (var i = 0; i < data['list'].length; i++) {
                        var item = data['list'][i];
                        html += '<li><div class="txt">';
                        html += ' <strong class = "name" > ' + item['to_member']['nickname'] + '</strong > ';
                        html += '<span class="date">' + item['add_time'] + '</span></div>';
                        html += '<div class="num">'+item['commission']+'</div>';
                        html += '</li>';
                    }
                    if (page.current > 1) {
