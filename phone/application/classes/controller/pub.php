...
class Controller_Pub extends Stourweb_Controller
{
    ...
    public function action_dopay()
    {
        ...
        //使用学分抵现,且当前订单没有使用过学分抵现功能.
        if (intval($usejifen) == 1 && $info['usejifen'] == 0)
        {
            $needjifen = $GLOBALS['cfg_exchange_jifen'] * $info['jifentprice']; //所需学分
            //当前登陆会员
            $member = Common::session('member');
            if ($member)
            {
                $memberid = $member['mid'];
                $tMsg= '预订产品' . $info['productname'] . '学分抵现消费学分' . $needjifen;
                $flag=Product::operateIntegral($memberid,$needjifen,$tMsg,Model_Member_Jifen_Log::TYPE_USE,Model_Member_Jifen_Log::ACTION_ORDER,array('orderid'=>$orderid));
                if ($flag)
                {
                    //如果会员学分扣除成功,则写订单信息,使用学分抵现.
        ...
        //再次查询数据库,判断是否使用学分成功,如果成功则重新计算价格.
 ...
 * */
    ...
    public function action_baidumap(){
        $longitude=Arr::get($_GET,'longitude',0);
        $latitude=Arr::get($_GET,'latitude',0);
        $this->assign('longitude', $longitude);
        $this->assign('latitude', $latitude);
        $this->display('pub/baidumap');
    }

    public function action_notes_modal(){
        $this->display('pub/notes_modal');
    }


    public function action_upload_image()
    {
        $input_name=Arr::get($_GET,'input_name','file');
        if ($input_name){
            echo json_encode($this->upload_file($input_name));
        }
