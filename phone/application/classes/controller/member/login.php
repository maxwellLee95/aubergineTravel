...
//        session_start();
        ...
        if (empty($code))
        {
            $url=$weixin->createOauthUrlForCode();
            header("location:{$url}");
            exit;
        }
        else
        {
            ...
            if (!empty($user))
            {
                ...
                else
                {
                    ...
                    $url = $this->cmsurl . "member/login/savebind?uname=" . urlencode($user['nickname']);
                    $url .= "&pwd=123456789&connectid=" . $openid . '&litpic='.urlencode($user['headimgurl']).'&from=weixin';
                    header("Location:{$url}");
                    exit;
                }
            }
    ...
    public function action_savebind()
    {
        ...
        if ($connectid){
            $member = array(
                ...
                'litpic'=>Arr::get($_GET, 'litpic'),
            );
            $sql="select * from sline_member where connectid='{$connectid}' ";
            $cur_member=DB::query(Database::SELECT,$sql)->execute()->current();
            if (!$cur_member['mid']){
                $member['from'] = empty($_GET['from']) ? $_SESSION['from'] : $_GET['from'];
                $member['distribution_id']=Model_Member::distributionID();
