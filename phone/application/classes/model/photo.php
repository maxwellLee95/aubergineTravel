...
class Model_Photo extends ORM
{
     ...
     * @return mixed
     */
    public static function photo_picture($id,$limit=null)
    {
    ...
    public static function photo_list($data)
    {
        $offset = $order = 0;
        $where = '';
        $filed = "a.id,a.aid,a.title,a.litpic,a.webid,a.description,a.shownum,a.addtime,a.modtime";
        extract($data);
        switch ($order)
        {
            //最新
    ...
    public static function get_photo_by_mid($mid){
        $line_sql="select DISTINCT productaid from sline_member_order WHERE typeid=1 AND memberid={$mid} ";
//            ." AND status=".Model_Member_Order::STATUS_FINISH;
        $sql="select * from sline_photo where line_id in({$line_sql})";
        $arr = DB::query(1, $sql)->execute()->as_array();
        return $arr;
    }
