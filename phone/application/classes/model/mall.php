...
class Model_Mall extends ORM
{
    ...
    //取得酒店的挂牌价格
    public static function get_sellprice($mallid)
    {
        $sql="SELECT MIN(price) AS minprice,sellprice FROM `sline_mall_sku` WHERE mallid='{$mallid}'";
    ...
    * 获取酒店最低价
    * */
    public static function get_minprice($mallid)
    {
        $sql = "SELECT MIN(price) AS price FROM `sline_mall_sku` WHERE mallid='$mallid' AND price!=0 && number>0";
    ...
    public static function search_result($params,$keyword,$pagesize='10')
    {
        ...
        if($rankId)
        {
            $where.= " AND a.mallrankid='$rankId'";
        }
        //价格区间
        if($priceId)
        {
            $priceArr = ORM::factory('mall_pricelist',$priceId)->as_array();
        ...
        if(!empty($destId))
        {
            $sql = "SELECT a.* FROM `sline_mall` a ";
            $sql.= "LEFT JOIN `sline_kindorderlist` b ";
            $sql.= "ON (a.id=b.aid AND b.typeid=".self::TYPE_ID." AND a.webid=b.webid AND b.classid=$destId)";
            $sql.= $where;
        ...
        else
        {
            ...
            $sql.= "ON (a.id=b.aid AND b.typeid=".self::TYPE_ID." AND a.webid=b.webid)";
            $sql.= $where;
            $sql.= "ORDER BY {$orderBy}a.modtime DESC,a.addtime DESC ";
            $sql.= "LIMIT {$offset},{$pagesize}";
        ...
        foreach($data as &$v)
        {
            ...
            $v['satisfyscore'] = Model_Comment::get_score($v['id'], self::TYPE_ID, $v['satisfyscore'], $v['commentnum']);//满意度
            $v['sellnum'] = Model_Member_Order_Product::getShowSellNum($v['id'],intval($v['bookcount'])); //销售数量
            $v['sellprice'] = Model_Mall::get_sellprice($v['id']);//挂牌价
            $v['price'] = Model_Mall::get_minprice($v['id']);//最低价
            $v['attrlist'] = Model_Mall_Attr::get_attr_list($v['attrid']);//属性列表.
            $v['url'] = Common::get_web_url($v['webid']) . "/malls/show_{$v['aid']}.html";
            $v['litpic'] = Common::img($v['litpic'],185,120);
            $v['mallrankid']=ORM::factory('mall_rank',$v['mallrankid'])->get('mallrank');
        }
    ...
    public static function suit_range_price($suitid,$sdate,$edate,$dingnum)
    {
        ...
        $sql = "SELECT price FROM`sline_mall_sku_price` ";
    ...
    {
        $time = time();
        $sql = "SELECT day FROM`sline_mall_sku_price` ";
    ...
    public static function check_suit_storage($suitid,$sdate,$edate,$dingnum)
    {
        $sdate = strtotime($sdate);
        $edate = strtotime($edate);
        $sql = "SELECT number FROM`sline_mall_sku_price` ";
        $sql.= "WHERE suitid='$suitid' AND day>=$sdate AND day<$edate";
        $ar = DB::query(1,$sql)->execute()->as_array();
        $flag = 1;
        foreach($ar as $row)
    ...
    public static function get_goods_new($offset,$row,$attrId=null)
    {
        $sql = "SELECT a.* FROM sline_mall a ";
        $sql.= "WHERE a.ishidden=0 ";
        $sql.= Product::get_attr_where($attrId);
    ...
    public static function _goods($list){
        if ($list){
            //对获取的数据进行初始化处理
            foreach($list as &$v)
            {
                $v['commentnum'] = Model_Comment::get_comment_num($v['id'],Model_Mall::TYPE_ID); //评论次数
                $v['sellnum'] = Model_Member_Order_Product::getShowSellNum($v['id'],intval($v['bookcount'])); //销售数量
                $v['sellprice'] = Model_Mall::get_sellprice($v['id']);//挂牌价
                $v['price'] = Model_Mall::get_minprice($v['id']);//最低价
                $v['attrlist'] = Model_Mall_Attr::get_attr_list($v['attrid']);//属性列表.
    ...
    public static  function get_detail_by_aid($aid){
        return ORM::factory('mall')
            ->where("webid=0 AND aid='{$aid}'")
            ->find()
            ->as_array();
    }
