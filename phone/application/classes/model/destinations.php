...
class Model_Destinations extends ORM
{
    ...
    * */
    ...
    static function get_parent(){
        $sql = "SELECT * FROM sline_destinations where isopen =1 and pid=0 order by displayorder";
        return DB::query(1, $sql)->execute()->as_array();
    }

    static function get_sub_by_pid($pid){
        $sql = "SELECT * FROM sline_destinations where isopen =1 and pid={$pid} order by displayorder";
        return DB::query(1, $sql)->execute()->as_array();
    }
