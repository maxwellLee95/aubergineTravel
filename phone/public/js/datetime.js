...
(function ($) {
    $.fn.date = function (options,Ycallback,Ncallback) {
        ...
        function resetInitDete(){
            ...
            initY = parseInt(that.val()) - opts.beginyear;
        ...
        function bindButton(){
            ...
            $("#dateconfirm").unbind('click').click(function () {
                var datestr = $("#yearwrapper ul li:eq("+indexY+")").html().substr(0,$("#yearwrapper ul li:eq("+indexY+")").html().length-1)+"-"+
                    ...
                    $("#daywrapper ul li:eq("+Math.round(indexD)+") .date_day").html().substr(0,$("#daywrapper ul li:eq("+Math.round(indexD)+") .date_day").html().length-1);
                if(datetime){
        ...
        function init_iScrll() {
            ...
            yearScroll = new iScroll("yearwrapper",{snap:"li",vScrollbar:false,
                onScrollEnd:function () {
                    ...
                    opts.endday = checkdays(strY,indexM);
        ...
        function CreateDateUI(){
            var str = ''+
                ...
                '<section>'+
                '<div id="datetitle"><h3 style="padding-top:13px">请选择日期</h3></div>'+
        ...
        function createDAY_UL(){
            ...
            for(var i=opts.beginday;i<=opts.endday;i++){
                ...
                day = new Date( opts.beginyear+indexY-1 +'-'+m+'-'+ i ).getDay();

                var dayHtml = '';
                dayHtml += '<li>';
                dayHtml +=     '<span class="date_day">'+i+'日'+'</span>';
                if( opts.show_week ){
                    var weekday = getWeek( day );
                    dayHtml += '<span class="weekday">'+weekday+'</span>';
                }
                dayHtml += '</li>';
                str+=dayHtml;
            }
