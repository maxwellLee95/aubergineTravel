...
<script>
    ...
    $(document).ready(function () {
        $('#user-bind-next').click(function () {
            $(this).parents('.login-bind-box').addClass('hide').siblings('.login-bind-box').removeClass('hide');
            //账号格式检测
            $('#user-check').blur(function () {
                ...
                if (valreg.test(val)) {
                    ...
                    !reg.test(val) ? msg = '邮箱号错误' : bool = true;
                }
                if (!bool) {
                    $('.user-error').html(msg);
                    return false;
        ...
        function user_bind(data) {
            $.post('/plugins/login_weixin/index/bind', data, function (rs) {
                if (rs.bool == true) {
                    ...
                    $('.login-bind-box').not('.hide').find('.user-error').html('绑定失败!');
                }
            }, 'json')
        }
    });
...
<div class="st-userlogin-box">
    <div class="st-login-wp">
        ...
        <div class="login-bind-box">
            ...
            <div class="login-bind-or">您在本站是否已有账户？</div>
            <div class="login-bind-xz"><a class="now-bind-btn fl" href="javascript:" id="user-bind-next">有，马上绑定</a><a
                    class="not-bind-btn fr user-bind" href="javascript:">没有，直接登录</a></div>
        </div>

        <div class="login-bind-box hide">
            <div class="login-bind-tit">绑定账号</div>
            <div class="login-account-key">
                ...
                <div class="login-bind-xz"><span class="bind-txt-btn">不绑了，<a class="user-bind" href="javascript:">直接登录</a></span><a
                        class="direct-login-btn fr" id="user-bind-success" href="javascript:">完成登录</a></div>
            </div>
        </div>
